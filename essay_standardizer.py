"""
论述题标准化处理器 - 使用OpenAI API
专门处理论述/案例类长答题目的格式标准化，严格按照prompt.md要求进行绝对保真处理
...
class EssayStandardizer:
    """论述题标准化器 - 使用OpenAI API，绝对保真处理"""
...
1. **识别起点 (忽略已处理的头部)**:
   * 在`[current_slice]`的开头，判断其内容是否是一个完整试题的开始。一个试题的明确开始标志包括：
     1) **以数字开头的行**（例如 `1. `、`2．`、`3（难度...）`等）；
     2) **不以数字开头**但本行或下一行包含难度标识的完整问句（例如行尾含`(难度: x)`、`（难度x）`、`（难度：x）`等）；
     3) 一个完整问句后，**紧随其后**出现`参考答案要点`或`【参考答案】`提示的结构。
   * 如果`[current_slice]`的开头明显属于上一题的**答案中间部分**（例如以项目符号`•`/`-`开头的列表、`参考答案`续行、纯缩进行等），你**必须忽略**这些内容，直到找到上述任一**明确的试题开始标志**为止。
...
4. **严格限定处理范围 (防止越界)**:
   ...
   * **绝对不能**处理任何在`[next_slice]`中完整开始的新试题（无论其是否以数字开头，只要满足上述开始标志，都视为新试题，应留待下一轮处理）。
...
请开始处理，严格按照上述规则进行绝对保真的试题识别和格式化。"""
    ...
    def parse_question_block(self, block: str) -> Optional[Dict]:
        try:
            ...
            return {
                'code': '',
                'question_type': extracted['question_type'] or '论述I',
    ...
    def create_excel_file(self, questions: List[Dict], output_path: str):
        ...
        write_excel(headers=headers, rows=rows, sheet_title="论述题", output_path=output_path)
    ...
    def process_standardized_to_excel(self, standardized_dir: str, output_dir: str = None) -> Optional[str]:
        ...
        excel_filename = f"论述题_{timestamp}.xlsx"
...
def main():
    ...
    standardizer = EssayStandardizer(
        api_key=api_key,
        api_base=api_base,
        model=model,
    )
    ...
    # 可调整chunk大小
    standardizer.config["lines_per_chunk"] = 150

    input_file = "question_processing_《数据安全管理员题库》（客观题）-20250713（提交版）/question_types/essay.md"
